  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      reset() {
        ...
        this.returnSpeed = 0.005 + Math.random() * 0.005; // Faster return
        this.friction = 0.95; // Less viscous
      ...
      update() {
        ...
        if (distMouse < interactionRadius) {
          ...
          const strength = 1.8; // High repulsion
          this.vx += (dxMouse / distMouse) * force * strength;
          this.vy += (dyMouse / distMouse) * force * strength;
          this.alpha = Math.min(this.baseAlpha + force * 0.5, 0.95);
        } else {
        ...
        this.alpha = Math.max(0.2, Math.min(1, this.alpha + flicker * 0.01));
      }

      draw() {
        const speed = Math.sqrt(this.vx * this.vx + this.vy * this.vy);
        const particleColor = '255, 140, 0';
        
        ctx.beginPath();
        if (speed > 1.5) {
            const angle = Math.atan2(this.vy, this.vx);
            const streakLen = speed * 2.2;
            ...
            // High-performance glow layer
            ctx.fillStyle = `rgba(${particleColor}, ${this.alpha * 0.2})`;
    ...
    const animate = () => {
      ...
      lerpMouse.x += (mouse.x - lerpMouse.x) * 0.25; // Snappier tracking
      lerpMouse.y += (mouse.y - lerpMouse.y) * 0.25;
