          ...
          >
            ...
            {isUploading && (
              <div className={`absolute inset-0 backdrop-blur-2xl rounded-[40px] flex flex-col items-center justify-center z-50 px-8 text-center transition-colors duration-700 ${isDark ? 'bg-[#050505]/98' : 'bg-white/98'}`}>
                 <div className={`w-14 h-14 border-4 rounded-full animate-spin mb-6 ${isDark ? 'border-white/5 border-t-orange-600 shadow-[0_0_20px_rgba(234,88,12,0.3)]' : 'border-slate-100 border-t-slate-900'}`} />
                 <h4 className={`text-xl font-black tracking-tighter transition-colors duration-700 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                   Uploading PDF...
                 </h4>
                 <p className={`text-xs mt-2 font-bold tracking-widest uppercase opacity-40 ${isDark ? 'text-orange-400' : 'text-slate-500'}`}>
                   Syncing with Intelligence
                 </p>
                 <div className="flex gap-2 mt-6">
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDark ? 'bg-orange-600' : 'bg-slate-900'}`} style={{ animationDelay: '0ms' }} />
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDark ? 'bg-orange-600' : 'bg-slate-900'}`} style={{ animationDelay: '200ms' }} />
                    <div className={`w-1.5 h-1.5 rounded-full animate-pulse ${isDark ? 'bg-orange-600' : 'bg-slate-900'}`} style={{ animationDelay: '400ms' }} />
                 </div>
              </div>
            )}
          </div>
